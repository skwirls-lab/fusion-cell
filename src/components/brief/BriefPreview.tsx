...
/** Live preview of a brief's markdown; [R-xxxx] renders as a chip that opens the report reader. */
...
export function BriefCitationChip({ number }: { number: string }) {
  ...
  return (
    <button
      ...
      title={`Open report ${number}`}
      onClick={() => openReport(number)}
      className="mx-0.5 inline-block rounded border border-concord/40 bg-concord/10 px-1 font-mono text-[10px] leading-4 align-baseline text-concord hover:bg-concord/25"
    >
      {number}
    </button>
  );
...
export function BriefPreview({ markdown }: { markdown: string }) {
  const source = useMemo(() => markdown.replace(CITATION_RE, (_, n: string) => `[${n}](${CITE_HREF}${n})`), [markdown]);
  const components = useMemo<Components>(() => ({
    a: ({ href, children }) => {
      if (href?.startsWith(CITE_HREF)) return <BriefCitationChip number={href.slice(CITE_HREF.length)} />;
